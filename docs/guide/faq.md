...
### 3. **Does Lemonade work on older AMD processors or non-Ryzen AI systems?**
   ...
   - **AMD Ryzen AI MAX+ (Strix Halo) on Linux**: in addition to the above, the experimental `vllm:rocm` backend is supported on gfx1151, and on AMD Instinct MI300X (gfx942, CDNA3) via a per-architecture release override (see [vLLM configuration](configuration/vllm.md))
